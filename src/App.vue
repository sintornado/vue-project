<template>
  <div id="app">
    <div class="left-block">
      <app-search 
                v-bind:search="findValue"
                v-on:input-search="changeFindValue($event)"
      >
      </app-search>
      <app-list
                v-for="(worker, i) in variant"
                v-bind:client="worker"
                :key="variant.id"
                v-bind:first="variant[i].general.firstName"
                v-bind:last="variant[i].general.lastName"
                v-bind:photo="variant[i].general.avatar"
                v-on:current-user-change="change($event)"
                v-on:delete-worker="deleteWorker($event)"
      >
      </app-list>
    </div>  
    <div class="right-block">
      <app-info v-bind:client="currentClient"></app-info>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    ...
    computed: {
        variant(){
          console.log('hello' + this.findValue);
          let h = this.findValue;
          return this.workers.filter((el) =>{
            if(h===''){
              return true;
            }else{
              return el.general.firstName.toLowerCase().indexOf(h) >-1;
            }
          })
        }
    },
    methods: {
        ...
        changeFindValue(search){
          this.findValue = search;
        },
        deleteWorker(client){
          this.workers.splice(client,1);
        }
    }, 
    components: {
      AppList: List,
